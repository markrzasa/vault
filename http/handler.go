...
func WrapListenersForClustering(lns []net.Listener, handler http.Handler, logger *log.Logger) func() ([]net.Listener, http.Handler, error) {
	...
	mux.HandleFunc("/cluster/forwarded-request", func(w http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			logger.Printf("[ERR] http/ForwardedRequestHandler: error reading forwarded request body: %v", err)
			respondError(w, http.StatusInternalServerError, fmt.Errorf("unable to read forwarded request body"))
			return
		}
		...
		freq.Header.Set(NoRequestForwardingHeaderName, "true")
